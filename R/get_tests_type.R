...
get_tests_type <-
  function(typeId,
    ...
    if (base::is.null(aToken) ||
      ...
      # Log Debug
      logger::log_debug(base::paste0("hawkinR/get_tests_type -> Temporary access token expires: ", as.POSIXct(token_exp)))
    }
    ...
    #-----#
    .Deprecated(new = "get_tests", msg = "`get_tests_type` has been deprecated. Use `get_tests` which has been expanded to handle all requests.")
    ...
    logger::log_debug(base::paste0(
      "hawkinR/get_tests_type -> ",reqPath$method, ": ", reqPath$headers$host, reqPath$path))
    ...
    if (status == 200) {
      ...
      if (count > 0) {
        ...
        trialMetrics <- replace_headers(df = trialMetrics, typeId = typeId)
