...
[![packageversion](https://img.shields.io/badge/Package%20version-`r "1.1.4"`-orange.svg?style=flat-square)](commits/master) 
...
-   `buildDB()` - Builds a data frame of prior test data from a set amount of prior days until today, then saves it to a file in in your designated location. It allows setting increments to control the number of days downloaded at a time for more efficient download speeds.

-   `syncDB()` - This function updates an existing test database by retrieving new tests and syncing updates for previous tests. It reads the current database from the specified file and returns an updated version.
